...
export class Plex extends PlexAPI {
	...
	public async sendQuery(query: string, type: number): Promise<PlexResponseQuery> {
	...
	public async getTracksFromName(name: string): Promise<PlexTrack[]> {
		const res: PlexResponseQuery = await this.sendQuery(name, PlexType.SONG);
	...
	public async getSongsFromName(name: string): Promise<PlexSong[]> {
		const tracks: PlexTrack[] = await this.getTracksFromName(name);
		const songs: PlexSong[] = []
	...
	public async getSongFromKey(key: string): Promise<PlexSong> {
		const queryHTTP = encodeURI(key);
		const res: PlexResponseQuery = await this.query(queryHTTP);
		console.dir(res);
		const track = dataToTrack(res.MediaContainer.Metadata[0]);
		return trackToSong(track);
	}

	private generateURLFromKey(key: string) : string {
...
function dataToTrack(data: PlexResponseMetadata): PlexTrack {
...
function trackToSong(track: PlexTrack): PlexSong {
	return {
		album: track.album,
		artist: track.artist,
		key: track.key,
